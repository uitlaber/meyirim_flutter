...
class _RegisterScreenState extends State<RegisterScreen> {
  ...
  void register() async {
    ...
    if (_formKey.currentState.validate()) {
      ...
      await attemptRegister(_formKey.currentState.value).then((value) {
        switch (value.statusCode) {
          case 200:
            var authData = json.decode(value.body);
            globals.userData = User.fromJson(authData['user']);
            setState(() {
              jwt = authData['token'];
              _isLoading = false;
            });
            break;
          default:
            errorMessage = json.decode(value.body)['error'];
            setState(() {
              _isLoading = false;
            });
        }
